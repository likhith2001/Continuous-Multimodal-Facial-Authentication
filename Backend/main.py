"""VSA Pipeline — End-to-end training orchestrator for deepfake detection.

Interactive CLI that drives the full pipeline: dataset selection, fake video
generation (Wav2Lip/FOMM/LivePortrait/Synthetic), optical flow extraction,
MAML meta-learning training, evaluation, and result logging to Excel.
"""
import os
import pandas as pd
...
def get_speakers(base_dir, dataset_name):
    """Discovers speaker IDs from the dataset directory structure.
    Returns a sorted list of speaker identifiers based on the dataset format."""
    speakers = []
...
def log_results_to_excel(results_list, log_path):
    """Appends training results to an Excel log file.
    Creates the file if it doesn't exist, otherwise appends new rows."""
...
def run_pipeline_for_dataset(dataset_name, mode_choice, reg_choice, current_tool_variant, root_dir, wav2lip_dir, fomm_dir, lp_dir, model_save_dir):
    """Runs the full pipeline for a single dataset + tool combination.
    Handles fake generation, optical flow extraction, MAML training, and evaluation."""
    ...
    if current_tool_variant == 'synthetic':
        ...
        modes = ['combined']
    elif current_tool_variant in ['ff_deepfakes', 'ff_face2face']:
        print(f"\n[Step 1] Using Pre-existing FaceForensics++ {current_tool_variant.upper()} Fakes...")
    ...
    for mode in modes:
        ...
        process_dataset_mode(
            ...
            target_speakers, video_limit, is_fake=False, tool_variant=current_tool_variant
        )

        if current_tool_variant != 'synthetic':
            output_dir_fake = os.path.join(flow_dir, "fake")
            if current_tool_variant == 'ff_deepfakes':
                 input_fake = os.path.join(base_dir, "manipulated_sequences", "Deepfakes", "c23", "videos")
                 process_dataset_mode(input_fake, output_dir_fake, mode, dataset_name, target_speakers, video_limit, True, tool_variant=current_tool_variant)
            elif current_tool_variant == 'ff_face2face':
                 input_fake = os.path.join(base_dir, "manipulated_sequences", "Face2Face", "c23", "videos")
                 process_dataset_mode(input_fake, output_dir_fake, mode, dataset_name, target_speakers, video_limit, True, tool_variant=current_tool_variant)
            else:
                 if current_tool_variant in ['wav2lip', 'both']:
                      process_dataset_mode(os.path.join(base_dir, "fake_dataset_w2l"), output_dir_fake, mode, dataset_name, target_speakers, video_limit, True, tool_variant='wav2lip')
                 if current_tool_variant in ['fomm', 'both']:
                      process_dataset_mode(os.path.join(base_dir, "fake_dataset_fomm"), output_dir_fake, mode, dataset_name, target_speakers, video_limit, True, tool_variant='fomm')
                 if current_tool_variant in ['liveportrait', 'both']:
                      process_dataset_mode(os.path.join(base_dir, "fake_dataset_lp"), output_dir_fake, mode, dataset_name, target_speakers, video_limit, True, tool_variant='liveportrait')
        ...
        # Pass flow_dir (not base_dir) and the mode argument to select the correct architecture
...
def main():
    """Interactive CLI entry point. Prompts for dataset, mode, tool, and region
    selections, then runs the training pipeline and logs results."""
    ...
    print("\nSelect Method:\n1. Wav2Lip (Legacy)\n2. FOMM (Legacy)\n3. BOTH (Legacy)\n4. FaceForensics++ Official: DeepFakes\n5. FaceForensics++ Official: Face2Face\n6. SYNTHETIC INCOHERENCE (New/Recommended)")
    ...
    elif tool_input == '4': tools_to_run = ['ff_deepfakes']
    elif tool_input == '5': tools_to_run = ['ff_face2face']
    elif tool_input == '6': tools_to_run = ['synthetic']
