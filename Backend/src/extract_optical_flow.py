"""Facial region extraction and optical flow computation.

Extracts lip/eye/combined facial regions from video frames using dlib landmarks,
then computes dense optical flow between consecutive frames for deepfake detection.
Supports GRID, MOBIO, and FaceForensics++ datasets with per-dataset preprocessing.
"""
import os
import cv2
import dlib
...
# Initialize dlib face detector and 68-point landmark predictor
detector = dlib.get_frontal_face_detector()
predictor = dlib.shape_predictor(PREDICTOR_PATH)

# Haar cascade as fallback face detector when dlib fails
haar_path = os.path.join(cv2.data.haarcascades, 'haarcascade_frontalface_default.xml')
face_cascade = cv2.CascadeClassifier(haar_path)

# Track extraction errors to avoid flooding console
extraction_errors = 0

def stabilize_face(frame, shape, desired_face_width=256):
    """Aligns and stabilizes a face using eye landmarks.
    Rotates and scales the frame so that the eyes are horizontally
    aligned and the face occupies a consistent region."""
...
def get_simple_crop(frame, points, pad=5):
    """Crops a rectangular region around the given landmark points with padding."""
    (x, y, w, h) = cv2.boundingRect(points)
    return frame[max(0, y-pad):y+h+pad, max(0, x-pad):x+w+pad]

def detect_face_robust(frame, dataset_name):
    """Multi-strategy face detection with fallbacks.
    Tries: (1) dlib HOG detector, (2) 90° rotations for MOBIO,
    (3) histogram equalization, (4) Haar cascade as last resort."""
...
def extract_region(frame, mode='lip', dataset_name='mobio', target_size=(64, 64)):
    """Extracts a facial region (lip, eye, or combined) from a single video frame.
    Uses face alignment for MOBIO; simple landmark cropping for other datasets.
    Returns a resized numpy array or None if no face is detected."""
...
def compute_optical_flow(video_path, output_folder, mode, dataset_name, filename_prefix=""):
    """Computes dense optical flow (Farneback) for a single video and saves as .npy.
    Extracts the specified facial region from each frame, computes frame-to-frame
    optical flow, and filters out low-motion videos for FaceForensics++."""
...
def process_dataset_mode(base_video_dir, base_flow_dir, mode, dataset_name, limit_speakers_paths=None, max_videos=None, is_fake=False, tool_variant=""):
    """Batch processes optical flow extraction for an entire dataset split.
    Iterates over all speakers, discovers their videos, and computes
    optical flow for each one. Supports real and fake video sources."""
    
    if tool_variant == 'ff_deepfakes':
        filename_prefix = "fake_df_"
    elif tool_variant == 'ff_face2face':
        filename_prefix = "fake_f2f_"
    else:
        filename_prefix = ""
    ...
    elif dataset_name == 'faceforensics' and is_fake and tool_variant in ['ff_deepfakes', 'ff_face2face']:
         if not os.path.exists(base_video_dir): return
         speakers = limit_speakers_paths if limit_speakers_paths else sorted(list(set([f.split('_')[0] for f in os.listdir(base_video_dir) if '_' in f and f.endswith('.mp4')])))
    else:
    ...
    if limit_speakers_paths and not (dataset_name == 'faceforensics' and is_fake):
    ...
    for spk_id in tqdm(speakers, desc=f"Processing {dataset_name.upper()} ({mode})"):
        ...
        if dataset_name == 'faceforensics' and not is_fake:
            ...
            prefix = spk_id + "_"
            for f in os.listdir(base_video_dir):
                if f.startswith(prefix) and f.endswith(('.mp4', '.avi')):
                    video_files.append(os.path.join(base_video_dir, f))
        else:
