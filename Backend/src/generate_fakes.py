"""Deepfake video generation using Wav2Lip, FOMM, and LivePortrait.

Automated pipeline to generate fake videos from real dataset speakers using
three deepfake tools. Each function handles one tool's end-to-end workflow:
audio extraction, subprocess invocation, and output file management.
Supports GRID, MOBIO, and FaceForensics++ datasets.
"""
import os
import subprocess
import sys
...
def extract_audio_from_video(video_path, audio_output_path):
    """Extracts audio track from a video file as 16kHz mono WAV using ffmpeg."""
    command = [
        "ffmpeg", 
        "-y",
        "-i", video_path,   
...
def restore_wav2lip_background(real_vid_path, fake_vid_path, out_vid_path, detector, predictor):
    """Blends Wav2Lip output with original video backgrounds.
    Replaces only the mouth region using dlib landmarks and a smooth mask
    to preserve the original background and reduce artifacts."""
...
def generate_fakes_wav2lip(
    ...
    """Generates fake videos using Wav2Lip (lip-sync deepfake).
    For each speaker, extracts audio, runs Wav2Lip inference, and optionally
    restores the original background using face landmark masking."""
...
def generate_fakes_fomm(
    ...
    """Generates fake videos using First Order Motion Model (face reenactment).
    Drives each source video with a randomly selected driving video
    from the dataset to create motion-transferred deepfakes."""
...
def generate_fakes_liveportrait(
    ...
    """Generates fake videos using LivePortrait (expression-driven face animation).
    Extracts a source frame, drives it with a random video, and renames
    the output to follow the project's naming convention."""
...
def generate_fake_videos(base_dir, wav2lip_dir, fomm_dir, liveportrait_dir, dataset_name, videos_per_user, limit_speakers, tool, predictor_path=None, step=1):
    """Entry point: orchestrates fake video generation across all selected tools.
    Discovers speakers from the dataset, filters by limit_speakers if provided,
    and dispatches to the appropriate per-tool generation function."""
