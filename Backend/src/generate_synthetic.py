"""Synthetic deepfake generation via temporal eye-lip desynchronization.

Creates 'fake' training data by time-shifting the eye and lip optical flow
streams relative to each other. This produces temporally inconsistent facial
movements that serve as negative examples for the deepfake detection model,
without requiring any external deepfake generation tools.
"""
import os
import cv2
import numpy as np
...
def generate_synthetic_dataset(base_dir, dataset_name, videos_limit=500):
    """Generates synthetic fake optical flow data by time-shifting eye/lip streams.
    For each real video, extracts eye and lip regions, applies a random temporal
    offset (5-15 frames) between them, computes optical flow, and saves as .npy."""
