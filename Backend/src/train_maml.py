"""MAML meta-learning training pipeline for deepfake detection.

Orchestrates the full training lifecycle: hyperparameter tuning (via Optuna),
MAML inner/outer loop training, evaluation with precision/recall/F1 metrics,
and production-grade visualization (ROC curves, confusion matrices, etc.).
"""
import os
...
# Import model architectures from tune_optuna module
OpticalFlowModel = tune_optuna.OpticalFlowModel
FusionOpticalFlowModel = tune_optuna.FusionOpticalFlowModel

class TaskGenerator:
    """Generates few-shot MAML episodes (support + query sets) for training.
    Identical to tune_optuna.TaskGenerator but extended with ff_deepfakes/ff_face2face filters."""
    ...
    def _process_flow(self, flow_data):
        """Normalizes an optical flow .npy file into a fixed-length tensor."""
        try:
    ...
    def _load_files_from_dir(self, path, lbl):
        """Lists .npy files from a speaker directory, filtered by tool type."""
        ...
        if lbl == 'fake':
            if self.tool_filter == 'synthetic':
                ...
                for f in all_files:
                    ...
                    elif self.tool_filter == 'ff_deepfakes' and 'fake_df_' in f: filtered.append(f)
                    elif self.tool_filter == 'ff_face2face' and 'fake_f2f_' in f: filtered.append(f)
    ...
    def load_n_samples(self, files, n_shots):
        """Loads n_shots optical flow tensors (oversamples if fewer files available)."""
        tensors = []
        if not files: return []
    ...
    def create_task(self):
        """Builds a MAML episode with support and query sets from random speakers."""
...
def generate_production_graphs(save_dir, run_name, history, test_data):
    """Generates 10 evaluation visualizations: loss curves, ROC, confusion matrix,
    precision-recall, confidence distributions, calibration, and class-wise accuracy."""
...
def evaluate_model(maml, task_gen, num_tasks=30):
    """Evaluates a MAML model across multiple few-shot tasks.
    Returns accuracy, precision, recall, F1, and meta-learning adaptation gain."""
...
# Select model architecture based on visual modality (combined = Two-Stream Fusion)
def run_training_pipeline(data_path, speakers, run_name="Default", tool="both", save_path=None, is_production=False, mode='combined'):
    ...
    # Architecture selection based on mode: combined uses Two-Stream Fusion, others use single-stream
    if mode == 'combined':
        print(f"   [Config] Using Fusion Architecture (Two-Stream) for {tool.upper()}")
        model = FusionOpticalFlowModel(dropout).to(device)
    else:
        print(f"   [Config] Using Standard 3D-CNN (Single Stream) for {tool.upper()} - {mode.upper()} Region")
