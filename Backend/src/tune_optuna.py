"""Hyperparameter tuning via Optuna for MAML-based deepfake detection.

Defines the neural network architectures (OpticalFlowModel for single-stream,
FusionOpticalFlowModel for two-stream lip+eye), the MAML task generator,
and the Optuna objective function for automated hyperparameter search.
"""
import os
...
# Training configuration for hyperparameter search
EPOCHS = 50      
TASKS_PER_EPOCH = 30
VAL_TASKS = 15

# Global variables set by run_hyperparameter_tuning() before Optuna optimization
CURRENT_OPTICAL_FLOW_PATH = ""
CURRENT_SPEAKERS = []
CURRENT_TOOL = "wav2lip" 

class OpticalFlowModel(nn.Module):
    """Single-stream 3D CNN for deepfake detection on optical flow.
    Can use either a custom Conv3D stack or a pre-trained R3D-18 backbone."""
...
class FusionOpticalFlowModel(nn.Module):
    """Two-stream fusion model for deepfake detection.
    Processes eye and lip optical flow through separate 3D CNN branches,
    then fuses their features for joint classification."""
    ...
    def forward(self, x):
        # Split input into eye region (top 32px) and lip region (bottom 32px)
        x_eyes = x[:, :, :, :32, :] 
        x_lips = x[:, :, :, 32:, :] 
...
class TaskGenerator:
    """Generates few-shot learning tasks (support + query sets) for MAML training.
    Each task samples real and fake optical flow data from selected speakers."""
    ...
    def _process_flow(self, flow_data):
        """Loads and normalizes an optical flow sequence to a fixed-length tensor."""
        try:
    ...
    def _load_files_from_dir(self, path, lbl):
        """Lists .npy flow files from a directory, filtered by tool type and label."""
    ...
    def load_n_samples(self, files, n_shots):
        """Loads n_shots optical flow tensors from file list (with oversampling if needed)."""
        tensors = []
        if not files: return []
    ...
    def create_task(self):
        """Builds a MAML episode: support set (s_x, s_y) and query set (q_x, q_y)
        using two randomly selected speakers."""
...
def objective(trial):
    """Optuna objective function. Trains a MAML model with sampled hyperparameters
    and returns mean validation accuracy across few-shot tasks."""
...
def run_hyperparameter_tuning(data_path, speakers, tool='wav2lip', n_trials=5):
    """Entry point: runs Optuna hyperparameter search and returns best parameters."""
